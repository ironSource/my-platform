...
function Windows() {
	...
	// dont report the wmic process itself in listProcesses
	this.hideWmicProcess = true
}
...
Windows.prototype.listProcesses = function(callback) {
	...
	child.execFile('wmic', ['process', 'get', 'ProcessId,CommandLine'], function(err, stdout, stderr) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (!command) continue
			if (hideWmic && command === 'wmic process get ProcessId,CommandLine') continue
