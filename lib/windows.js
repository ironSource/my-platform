...
Windows.prototype._launch = function (app, user) {
	...
	if (app.args) {
		for (var i = 0; i < app.args.length; i++) {
			if (app.args[i].indexOf(' ') > -1) {
				commandArgs.push('"' + app.args[i]  + '"')
			} else {
				commandArgs.push(app.args[i])
			}
		}
	...
	app.args.push(command + ' ' + commandArgs.join(' '))

	if (app.waitForChildProcess) {
		app.args.push('--enableChildProcessWatch')
	}
